...
class Card():
	...
	def __str__(self):
		# Print string representation of a Card()
		# e.g. Two of Hearts
...
class Deck():
	...
	def __str__(self):
		...
		for card in self.deck:
			deck_comp += '\n ' + card.__str__() # Call __str__ function of a Card()
		return 'Current deck composition:' + deck_comp
...
class Hand():
	...
	def add_card(self, card):
		# Pass a Card(suit, rank) from Deck.deal()
		self.cards.append(card)
		self.value += values[card.rank]
		# Track number of Aces in Hand
		if card.rank == 'Ace':
			self.aces += 1
...
def hit_or_stand(deck, hand):
	...
	while True:
		...
		if x[0].lower() == 'h':
			...
			print("Sorry wrong input! Please try again! Enter 'H' or 'S': ")
			continue
		break
